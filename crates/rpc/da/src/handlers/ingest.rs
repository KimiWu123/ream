...
use ream_da::{
    column::{CandidateBlock, CandidateColumn, DaContext},
    id::DaColumnId,
};
...
/// SSZ body of `POST /da/v0/ingest/block/{block_root}`: at most one payload
...
/// Query string of `POST /da/v0/ingest/block/{block_root}`.
#[derive(Deserialize)]
...
/// `POST /da/v0/ingest/block/{block_root}?slot={slot}` — admit a whole block's
...
/// same load-shedding [`DaIngestHandle::try_submit_block`] path as the
...
pub async fn post_ingest_block(
    ...
    let block = CandidateBlock::new(block_root, DaContext { slot: query.slot }, columns)
