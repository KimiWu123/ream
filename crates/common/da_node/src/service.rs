...
use ream_da::{
    ...
    id::DaColumnId,
    reconstruction::DaReconstructor,
...
use crate::ingest::{DaWorkItem, ReconstructionRequest, RetentionHint};
...
impl DaVerificationService {
    ...
    pub async fn run(mut self) {
        ...
        while let Some(item) = self.receiver.recv().await {
            match item {
                ...
                DaWorkItem::CandidateBlock(candidate) => {
                    self.process_candidate_block(candidate).await
                }
                DaWorkItem::Retention(hint) => self.process_retention(hint).await,
                DaWorkItem::Reconstruction(request) => self.process_reconstruction(request).await,
            }
        }
    ...
    fn recover_block(
        store: &dyn DaWriteStore,
        reconstructor: &dyn DaReconstructor,
        block_root: B256,
        availability: DaAvailability,
    ) -> Option<CandidateBlock> {
        let mut held = Vec::new();
        for index in availability.held_indices() {
            let id = DaColumnId::new(block_root, index).expect("held indices are always in range");
            match store.get(&id) {
    ...
    fn maybe_schedule_reconstruction(&self, block_root: B256, before: &DaAvailability) {
        ...
        self.executor.spawn(async move {
            ...
            let _ = sender
                .send(DaWorkItem::Reconstruction(ReconstructionRequest {
                    block_root,
                }))
                .await;
        });
...
mod tests {
    ...
    use alloy_primitives::B256;
    use ream_da::{
        column::{CandidateBlock, CandidateColumn, DaContext, VerifiedColumn},
    ...
    impl DaReconstructor for PanicReconstructor {
        fn reconstruct(
            &self,
    ...
    impl DaVerifier for CountingVerifier {
        fn verify(&self, candidate: CandidateColumn) -> Result<VerifiedColumn, ValidationError> {
    ...
    fn block_batch_skips_already_held_columns() {
        ...
        store
            .put(VerifiedColumn::new_unchecked(
                DaColumnId::new(block_root, 1).expect("valid index"),
                DaContext { slot: 40 },
                vec![1],
            ))
            .expect("seed store");
    ...
    fn block_batch_stores_survivors_when_some_columns_are_rejected() {
        ...
        executor.runtime().block_on(async move {
            ...
            let rejected = DaColumnId::new(block_root, 3).expect("valid index");
    ...
    impl DaReconstructor for FillMissingReconstructor {
        fn reconstruct(
            &self,
    ...
    async fn wait_until_complete(store: &DaFileStore, block_root: B256) {
    ...
    fn incomplete_block_self_heals_through_the_verify_gate() {
        ...
        executor.runtime().block_on(async move {
            ...
            let recovered = DaColumnId::new(block_root, 100).expect("valid index");
    ...
    fn a_trickled_column_crossing_the_threshold_triggers_recovery() {
        ...
        for index in 0..NUMBER_OF_COLUMNS / 2 - 1 {
            store
                .put(VerifiedColumn::new_unchecked(
                    DaColumnId::new(block_root, index).expect("valid index"),
                    DaContext { slot: 40 },
                    vec![index as u8],
                ))
                .expect("seed store");
