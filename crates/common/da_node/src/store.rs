...
impl DaFileStore {
    ...
    pub fn new(da_root: PathBuf) -> Result<Self, DaStoreError> {
        ...
        let retention_floor = Self::load_retention_floor(&da_root);
        let store = Self {
            root: da_root,
    ...
    fn persist_retention_floor(&self, floor: u64) -> Result<(), DaStoreError> {
...
impl DaReadStore for DaFileStore {
    ...
    fn availability(&self, block_root: B256) -> DaAvailability {
        ...
        DaAvailability::new(held, ALL_COLUMNS_MASK)
    }
...
mod tests {
    ...
    use super::{BlockEntry, DaFileStore, RETENTION_FLOOR_FILE};
    ...
    fn retention_floor_survives_reopen() {
        ...
        let reopened = DaFileStore::new(root.clone()).expect("reopen store");
    ...
    fn retention_hint_below_the_floor_never_lowers_it() {
        ...
        let reopened = DaFileStore::new(root.clone()).expect("reopen");
