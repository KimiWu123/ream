//! A minimal HTTP client for the DA node's `/da/v0` RPC — the same wire
...
/// endpoints (the DA node sheds load when its verification queue is full; that
...
///
/// Mirror of `WireIndexedPayload` in `ream-rpc-da` — keep the two in sync (a
...
/// SSZ body of `POST /da/v0/ingest/block/{block_root}`.
...
impl DaClient {
    ...
    pub async fn health(&self) -> Result<Value> {
        self.get_json(&format!("{}/da/v0/health", self.base)).await
    }

    /// `GET /da/v0/availability/{block_root}`
    pub async fn availability(&self, block_root: B256) -> Result<Value> {
        self.get_json(&format!("{}/da/v0/availability/{block_root}", self.base))
            .await
    }

    /// `POST /da/v0/ingest` — submit one column payload (hex-encoded SSZ) for
    /// verification. Backs off and retries while the DA node reports overload.
    pub async fn ingest(
        &self,
        block_root: B256,
        index: u64,
        ...
        let url = format!("{}/da/v0/ingest", self.base);
        let body = json!({
        ...
        for _ in 0..OVERLOAD_RETRIES {
            ...
            match response.status() {
                ...
                status => {
                    ...
                    bail!("DA node rejected column {index}: {status}: {body}");
                }
            }
        }
        bail!("DA node still overloaded after {OVERLOAD_RETRIES} retries");
    }

    /// `POST /da/v0/ingest/block/{block_root}?slot={slot}` — submit a whole
    /// block's columns as one SSZ batch: raw payload bytes, no hex, no JSON,
    /// one HTTP round-trip. Backs off and retries while the DA node reports
    ...
    pub async fn ingest_block(
        ...
        let url = format!("{}/da/v0/ingest/block/{block_root}?slot={slot}", self.base);
        for _ in 0..OVERLOAD_RETRIES {
            let response = self
                .http
                .post(&url)
            ...
            match response.status() {
                ...
                status => {
                    let body = response.text().await.unwrap_or_default();
                    bail!("DA node rejected the batch: {status}: {body}");
                }
            }
        }
        bail!("DA node still overloaded after {OVERLOAD_RETRIES} retries");
    ...
    async fn get_json(&self, url: &str) -> Result<Value> {
        let response = self
            ...
            .send()
            .await
            .with_context(|| format!("GET {url} — is the DA node running?"))?;

        let status = response.status();
        if !status.is_success() {
            let body = response.text().await.unwrap_or_default();
            bail!("DA node returned {status} for {url}: {body}");
        }
