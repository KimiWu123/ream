...
//! node in sight, for exercising the DA pipeline's success path anywhere.
//!
//! Ported from the `feat/da-client` branch's `generate_sidecars` example. The
//! DA node deliberately does not verify proposer signatures or block
...
pub fn build_synthetic_sidecars(
    ...
    // A zeroed signature is fine: the DA node does not verify proposer
