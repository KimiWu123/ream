...
//!   ream-da-tool feed --beacon-url <url> [block_id] [--columns 0,1,2] [--wait 30] [--per-column]
//!   ream-da-tool generate [--slot 1] [--blobs 1] [--out dir] [--per-column]
...
use crate::{beacon::build_column_sidecars, da::DaClient, generate::build_synthetic_sidecars};
...
enum Command {
    ...
    Generate {
        ...
        /// limit at `slot`'s epoch or the DA node will reject the columns.
        #[arg(long, default_value_t = 1)]
        blobs: usize,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Feed {
            ...
            submit_and_wait(&da, block_root, slot, &selected, wait, per_column).await?;
        }
        Command::Generate {
            slot,
            blobs,
            ...
            match out {
                ...
                None => submit_and_wait(&da, block_root, slot, &selected, wait, per_column).await?,
            }
        }
    }
...
async fn submit_and_wait(
    ...
    if per_column {
        for sidecar in sidecars {
            da.ingest(block_root, sidecar.index, slot, &payload_hex(sidecar))
                .await
        ...
        let columns: Vec<(u64, Vec<u8>)> = sidecars
            ...
            .collect();
        da.ingest_block(block_root, slot, &columns)
            .await
...
/// Write one ingest-ready JSON request body per column, for driving the DA
...
fn write_vectors(
    ...
    println!(
        "submit one with: curl -X POST <da-url>/da/v0/ingest \
