...
A small end-to-end driver for the standalone DA node. It speaks the same
`/da/v0` RPC the beacon-side feeder will speak, so it doubles as living
...
# What does the DA node hold for a block?
ream-da-tool availability <0x-block-root>
...
# as ONE SSZ batch request, and wait until the DA node has verified and
# stored every one.
ream-da-tool feed --beacon-url <beacon-api-url> [block_id]
...
and a self-built commitments inclusion proof — valid because the DA node
...
Run the DA node with `--ephemeral` when experimenting so test columns don't
...
```bash
cargo build -p ream -p ream-da-tool

# 1. start the DA node (loopback only)
./target/debug/ream -v 4 da_node

# 2. feed it a real block (pick one with blobs)
./target/debug/ream-da-tool feed \
...
./target/debug/ream-da-tool availability 0xe4bb...a2d5
```
