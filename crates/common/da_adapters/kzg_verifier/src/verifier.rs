...
/// with two roles sharing one trusted setup: [`DaVerifier`] decodes a
...
/// [`DaReconstructor`] erasure-recovers a block's missing columns from any held
/// half.
#[derive(Debug, Clone)]
...
impl DaVerifier for KzgAdapter {
...
impl DaReconstructor for KzgAdapter {
    /// PeerDAS recovery
    fn reconstruct(
        &self,
        ...
        for sidecar in all_columns {
            ...
            recovered.push(CandidateColumn {
                id: DaColumnId::new(block_root, sidecar.index)?,
                context,
