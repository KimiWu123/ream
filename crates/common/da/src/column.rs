...
use crate::{error::ValidationError, id::DaColumnId};
...
impl CandidateBlock {
    ...
    pub fn new(
        ...
        for (index, _) in &columns {
            ...
            // of that invariant.
            DaColumnId::new(block_root, *index)?;
            let bit = 1u128 << *index;
            if seen & bit != 0 {
    ...
    pub fn into_parts(self) -> (B256, DaContext, Vec<(u64, Vec<u8>)>) {
...
mod tests {
    ...
    use super::{CandidateBlock, DaContext};
    ...
    fn candidate_block_rejects_an_empty_batch() {
        assert_eq!(
            CandidateBlock::new(B256::ZERO, DaContext::default(), vec![]),
    ...
    fn candidate_block_rejects_an_out_of_range_index() {
        ...
        assert!(matches!(
            CandidateBlock::new(B256::ZERO, DaContext::default(), columns),
    ...
    fn candidate_block_round_trips_through_parts() {
        ...
        let block = CandidateBlock::new(B256::ZERO, DaContext { slot: 5 }, columns.clone())
